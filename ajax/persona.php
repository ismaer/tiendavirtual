...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if(empty($idpersona)) {
			...
			echo $rspta ? "Persona registrada": "No se pudo registrar la persona";
		}
		else{
			$rspta=$persona->editar($idpersona,$tipo_persona,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email);
			echo $rspta ? "Persona actualizada": "Esta persona no se pudo Actualizar";
		}
	break;
